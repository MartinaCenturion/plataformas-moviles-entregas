...
function calcularPromedio(listaDeNumeros) {
    let sumaTotal = listaDeNumeros.reduce((a, b) => a + b, 0);
 ...
 * 
 * Ejemplos:
 * - filtrarSoloTruthy(["Hola", "", null, 1, 0, -1, undefined, [], {}]) retorna ["Hola", 1]
 */
...
function enumerarLista(listaDePalabras) {
    let enumeracion = "";
    listaDePalabras.forEach((palabra, index) => {
        if (index === listaDePalabras.length - 1) {
            enumeracion += `y ${palabra}.`;
        } else if (index === listaDePalabras.length - 2) {
            enumeracion += `${palabra} `;
        } else {
            enumeracion += `${palabra}, `;
        }
    });
    return enumeracion.charAt(0).toUpperCase() + enumeracion.slice(1);
}
